...
pub fn decode_and_resize_u8(
    ...
    Ok(crate::image_io_pixels::bilinear_resize_rgb_hwc(
...
fn decode_png(bytes: &[u8], path: &Path) -> Result<(Vec<u8>, usize, usize), EvalError> {
    ...
    let rgb =
        crate::image_io_pixels::expand_to_rgb(&buf, h, w, info.color_type, info.bit_depth, path)?;
    Ok((rgb, h, w))
}
...
fn decode_jpeg(bytes: &[u8], path: &Path) -> Result<(Vec<u8>, usize, usize), EvalError> {
    ...
    let rgb = match info.pixel_format {
        ...
        jpeg_decoder::PixelFormat::L8 => crate::image_io_pixels::grayscale_to_rgb(&pixels),
        other => {
