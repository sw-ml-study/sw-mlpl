...
pub const PROGRESS_NOTES: &[ProgressNote] = &[
    ...
    ProgressNote {
        ...
        line_idx: 56,
        heading: "Training the Vision Transformer (~1-3 min)",
        body: "50 full-batch Adam steps on 16 images (double the quick demo). The trained model is then used to predict labels for all 16 images and renders a labeled gallery showing actual vs predicted for each one.",
    },
];
...
pub const DEMOS: &[Demo] = &[
    ...
    Demo {
        ...
        lines: &[
            ...
            "train 50 { adam(cross_entropy(apply(classifier, take(apply(attn, reshape(apply(linear_p, reshape(patchify(X, 16), [256, 768])), [16, 16, 128])), 1, 0)), Y), [linear_p, attn, classifier], 0.001, 0.9, 0.999, 0.00000001); cross_entropy(apply(classifier, take(apply(attn, reshape(apply(linear_p, reshape(patchify(X, 16), [256, 768])), [16, 16, 128])), 1, 0)), Y) }",
            "loss_curve(last_losses)",
