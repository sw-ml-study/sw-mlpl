...
pub const DEMOS: &[Demo] = &[
    ...
    Demo {
        ...
        intro: "Trained 4-head [[ViT (Vision Transformer)]] on a balanced 8-image cat/dog subset, with per-head attention maps rendered after training. Same architecture as the single-head quick demo but [[Multi-head attention]] (`attention(128, 4)`) and an [[attention_weights (builtin)]] + [[heatmap_grid (viz type)]] viz at the end. Browser-friendly: 30 adam steps on 8 images. Compare the heatmap_grid output here with the UNTRAINED 'ViT Multi-Head Attention Pattern' demo -- the four heads start identical and end specialized. (Click any underlined term to open its glossary entry.)",
    ...
    Demo {
        ...
        intro: "Same trained 4-head [[ViT (Vision Transformer)]] as the multi-head demo above, but the per-head attention is rendered OVER the test image instead of as an abstract [16, 16] matrix. Each cell of the output is the same cat photo with one head's attention pattern painted on top: bright yellow patches are what THAT head attends to, dark purple patches are ignored. After 30 adam steps the four cells should look DIFFERENT -- the heads start identical and end specialized. Uses the new [[attention_overlay (viz type)]] viz. (Click any underlined term to open its glossary entry.)",
