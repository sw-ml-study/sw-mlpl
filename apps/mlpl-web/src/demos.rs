...
pub const DEMOS: &[Demo] = &[
    ...
    Demo {
        ...
        lines: &[
            ...
            "K0 = matmul(K, S0)",
            "V0 = matmul(V, S0)",
            "W0 = softmax(matmul(Q0, transpose(K0)) / sqrt(d_k), 1)",
            "svg(W0, \"heatmap\")",
            "out0 = matmul(W0, V0)",
            "Q1 = matmul(Q, S1)",
            "K1 = matmul(K, S1)",
            "V1 = matmul(V, S1)",
            "W1 = softmax(matmul(Q1, transpose(K1)) / sqrt(d_k), 1)",
            "svg(W1, \"heatmap\")",
            "out1 = matmul(W1, V1)",
            "out_concat = matmul(out0, transpose(S0)) + matmul(out1, transpose(S1))",
            "Wo = randn(7, [d_model, d_model])",
            "final_out = matmul(out_concat, Wo)",
            "shape(final_out)",
        ],
    },
    Demo {
