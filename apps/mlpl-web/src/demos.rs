...
pub const DEMOS: &[Demo] = &[
    ...
    Demo {
        ...
        lines: &[
            "# 4-point boolean inputs, shared across all five gates.",
            "X = [[0,0],[0,1],[1,0],[1,1]]",
            "n = 4",
            "lr = 1.0",
            "# --- AND: only [1,1] is positive. Linearly separable. ---",
            "y = [0, 0, 0, 1]",
            "w = zeros([2])",
            "b = 0",
            ...
            "# --- OR: only [0,0] is negative. Linearly separable. ---",
            "y = [0, 1, 1, 1]",
            "w = zeros([2])",
            "b = 0",
            "repeat 400 { z = matmul(X, reshape(w, [2, 1])) + b; pred = sigmoid(z); dz = pred - reshape(y, [4, 1]); dw = reshape(matmul(transpose(X), dz), [2]) / n; db = mean(dz); w = w - lr * dw; b = b - lr * db }",
            ...
            "# --- NAND: only [1,1] is negative. Linearly separable. ---",
            "y = [1, 1, 1, 0]",
            "w = zeros([2])",
            "b = 0",
            "repeat 400 { z = matmul(X, reshape(w, [2, 1])) + b; pred = sigmoid(z); dz = pred - reshape(y, [4, 1]); dw = reshape(matmul(transpose(X), dz), [2]) / n; db = mean(dz); w = w - lr * dw; b = b - lr * db }",
            ...
            "# --- NOR: only [0,0] is positive. Linearly separable. ---",
            "y = [1, 0, 0, 0]",
            "w = zeros([2])",
            "b = 0",
            "repeat 400 { z = matmul(X, reshape(w, [2, 1])) + b; pred = sigmoid(z); dz = pred - reshape(y, [4, 1]); dw = reshape(matmul(transpose(X), dz), [2]) / n; db = mean(dz); w = w - lr * dw; b = b - lr * db }",
            ...
            "# --- XOR: positives on one diagonal, negatives on the other.",
            "# Not linearly separable. The same training procedure converges,",
            "# but it converges to the LEAST-bad linear fit -- which is",
            "# roughly the constant 0.5 surface. Look at how flat / undecided",
            "# the rendered surface is, and how the training points have",
            "# colors that do not match the surface beneath them.",
            "y = [0, 1, 1, 0]",
            "w = zeros([2])",
            "b = 0",
            "repeat 400 { z = matmul(X, reshape(w, [2, 1])) + b; pred = sigmoid(z); dz = pred - reshape(y, [4, 1]); dw = reshape(matmul(transpose(X), dz), [2]) / n; db = mean(dz); w = w - lr * dw; b = b - lr * db }",
            ...
            "# Final w and b after the XOR run. Both near zero -- the model",
            "# correctly figured out there's nothing useful a line can do.",
            "w",
            "b",
        ],
    },
