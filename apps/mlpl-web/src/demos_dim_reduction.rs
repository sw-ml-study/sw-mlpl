...
pub const UMAP_VS_PCA: Demo = Demo {
    name: "UMAP vs PCA",
    intro: "Two-moons in 2-D, embedded in 5-D with three low-variance noise dimensions, then projected back to 2-D by both [[PCA (Principal Component Analysis)]] (linear) and [[UMAP]] (non-linear / manifold-aware). PCA picks the top two axes of variance; UMAP builds a k-NN graph over the full 5-D distances and runs SGD on the fuzzy simplicial set. **Caveat (saga 33):** MLPL's UMAP currently uses a simplified Student-t (a=1, b=1) curve with a tight coordinate clamp, no spectral init, and no min_dist-driven a/b fit. On this moons fixture that means UMAP separates the two classes but does NOT preserve the crescent shape -- you will see PCA's nicer arc reconstruction and UMAP's blockier layout. A follow-up step replaces the clamp + fits a/b properly so the arcs return; until then PCA wins this particular comparison.",
    takeaway: "PCA reconstructs the moon arcs because the noise dimensions carry less variance than the moons themselves -- you can see crescents. UMAP separates the two classes via the k-NN graph but the simplified-impl layout looks blockier than the McInnes reference would produce. The legend in each scatter maps integer label to color (saga 33 step 037 added the legend; named labels deferred until svg() supports StrList aux).",
    lines: &[
