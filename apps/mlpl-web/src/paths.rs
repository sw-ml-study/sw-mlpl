...
//!
//! A `LearningPath` is just a list of `Stop`s, each of which
...
//! renders each stop with a path-specific "why this is here"
...
pub const PATHS: &[LearningPath] = &[
    LearningPath {
        title: "Zero to LLM",
        blurb: "The spine: orientation -> arrays -> classifiers -> MLP -> autograd -> attention -> transformer -> tiny LM. Twelve stops; assume zero ML background.",
        stops: &[
            Stop::Lesson {
                title: "What is ML, and why are we here?",
                why: "Set the destination first. Every later stop is a variation of the same recipe (data, model, loss, gradient descent).",
            },
            Stop::Lesson {
                title: "Hello Numbers",
    ...
    LearningPath {
        ...
        stops: &[
            Stop::Note {
                ...
                body: "Each stop is one diagram. The blurb says what slice of MLPL covers the same ground (or notes that we have only the glossary entry, not a runnable demo). Skim the whole path first to get the lay of the land; come back to specific diagrams when you start a topic.",
            },
            Stop::Diagram {
    ...
    LearningPath {
        ...
        blurb: "The from-scratch attention bundle: see the diagram, then build the same thing in MLPL with no model-DSL shortcuts. Nine stops; alternates diagrams and lessons.",
        stops: &[
            Stop::Diagram {
                slug: "12_attention",
