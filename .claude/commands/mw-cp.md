...
- `markdown-checker -f "**/*.md"` -- only if any `.md` changed.
  Markdown is ASCII-only. (On some hosts the binary is installed as
  `sw-markdown-checker`.) Gate on the files YOU touched: the
  repo-wide run currently reports pre-existing failures in
  `.agentrail-archive/` (autogenerated), `vendor/`, the
  agentrail-managed CLAUDE.md block, and historical CHANGES.md
  subjects -- those are owned by a cleanup spike, not your commit.
  Any file you hand-edited must pass.
