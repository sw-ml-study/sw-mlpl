...
<!-- sw-mlpl repo-specific policy below: relocated OUT of the
     agentrail-managed briefing block on 2026-08-12 so that a future
     `agentrail instructions apply` cannot overwrite it. Keep
     repo-specific rules here (outside the markers), not inside them. -->

## Agentrail session transcripts are gitignored (sw-mlpl)

`.agentrail/sessions/*.jsonl` and `.agentrail-archive/*/sessions/*.jsonl`
are raw Claude Code event logs (tens of MB, append-only). They are
