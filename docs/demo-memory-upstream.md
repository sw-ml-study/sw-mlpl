...
| 2 | Fixed-width unsigned ints + bit ops | QUEUED (`fixed-width-ints-bitops`) | Masks, shifts, popcount, explicit conversions. Blocks Swiss control bytes, compact Bloom filters, Hamming indexes, binary sparse retrieval. |
...
story that distinguishes demo-memory. Fixed-width ints + bit
ops next -- the largest classical-structures unlock (Swiss,
