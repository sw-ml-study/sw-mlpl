...
**Saga 19 -- LLM-as-tool REST integration.** Saga 22 (v0.15.0)
just shipped the feasibility / resource estimator
...
check. Saga 19 adds an `llm_call` builtin that issues HTTPS
requests to an external chat-completions endpoint and parses
the JSON response into an MLPL string -- letting MLPL programs
use a hosted LLM as a composable tool (summarization,
classification, tool-use in a RAG pipeline). The 3-step plan
survives in `.agentrail-archive/mlpl-llm-rest-20260424T095011/`
