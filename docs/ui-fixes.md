...
- [ ] **REPL output collapses in 3D (#3).** When 3D is open the
...
- [ ] **Reset 3D camera on toggle.** Toggling 3D off/on does not
...
- [ ] **Escape-close parity.** Closing the inspector with Escape
...
- [ ] **Dialog prev/next carousel.** While a dialog is open,
...
- [ ] **Quiet script mode + flags.** Script mode echoes every
...
- [~] **CLI demo scripts.** `scripts/examples/{sum,stats}.mlpl`
