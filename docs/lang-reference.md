...
| `reduce(:op, a, axis)` | 3 | Same, restricted to one or more axes. `axis` is a scalar position (`2`), a vector of positions (`[2, 3]`), or a string naming labeled axes -- one (`"channel"`) or a comma-separated list (`"channel,kernel_y,kernel_x"`). Multiple axes collapse high-index first, and the surviving axes keep their labels, so named reductions chain. `reduce(:add, T, "channel")` needs `T` to carry axis labels (see `label`). |
