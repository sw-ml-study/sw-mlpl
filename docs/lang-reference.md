...
| `concat(a, b, axis)` | 3 | Axis-aware concat for any rank. Both inputs must agree on every dim except `axis` (sizes add); the forward accepts any `axis` in `[0, rank)`. Differentiable on the tape; the backward splits the upstream gradient at the seam. The autograd backward currently supports `axis` in `{0, 1}`; lifting the backward to match the forward is in flight (saga 30 step 002). |
