...
| `parse_json(s)` | 1 | JSON text to a typed MLPL value -- the inverse of the test-event encoding: objects become records, strings stay strings (Unicode and escapes exact, surrogate pairs included), numbers become scalars, homogeneous arrays become vectors or string lists (mixed/nested arrays are an err -- MLPL arrays are flat), `true`/`false` become 1/0, `null` becomes the empty vector. Returns `ok(value)` or `err(message with the byte position)`, so `event = parse_json(line)?` composes. |
...
| `parse_toml(text)` | 1 | Decode the TOML config subset into a record, RESULT-based: `ok(record)` / `err(message with line)`. Line-oriented: `#` comments and blank lines are skipped, `[table]` / dotted `[table.sub]` headers nest records, `key = value` assigns into the current table. Values reuse the JSON value parser -- integer, float, `true`/`false` (to 1/0), basic string, and homogeneous arrays of numbers or strings. A trailing `# comment` after a value is allowed. Round-trips with `to_toml` for the supported forms: `parse_toml(unwrap(to_toml(record)))`. |
