...
| `to_json(v)` | 1 | The deterministic encode half of `parse_json`, RESULT-based like it: `ok(json_string)` / `err(message)`. Records become objects with SORTED keys, rank-1 arrays flat lists (higher rank nests by shape), strings JSON-escaped with exact Unicode, integers print bare, `ok(x)`/`err(e)` become `{"ok":true,"value":x}` / `{"ok":false,"error":e}`. Returns `err(...)` for a non-data kind (models, tokenizers, generation state, partials, references, device tensors) or a NON-FINITE number (`NaN`/`±Inf` have no JSON form) -- never a hard error and never invalid JSON. Scalars, rank-1 arrays, strings, string lists, and records of those round-trip through `parse_json` (both sides Result-based: `parse_json(unwrap(to_json(v)))`). |
