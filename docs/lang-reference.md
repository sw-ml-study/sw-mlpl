...
| `write_bytes(path, bytes)` | 2 | Sandboxed raw-byte write (creates or replaces): `bytes` a rank-`<=1` array whose cells are integers `0..=255` (an out-of-range or non-integer cell is a LOUD error naming it); `ok(1)` / `err(...)`. Store encoded data as raw bytes: `write_bytes(p, tokenize_bytes(unwrap(to_json(v))))`. |
| `write_atomic(path, value)` | 2 | Sandboxed CRASH-SAFE write: `value` a string (its UTF-8 bytes) or a byte array (as in `write_bytes`). Writes a hidden sibling temp file then `rename`s it over the target, so a concurrent reader sees the old file or the whole new one -- never a torn write (atomic on a POSIX same-filesystem rename); the temp is removed on failure. `ok(1)` / `err(...)`. Durable persist: `write_atomic(p, unwrap(to_json(v)))`. |
