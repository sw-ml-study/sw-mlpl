...
| `read_bytes(path)` | 1 | Sandboxed raw-byte read: `ok(bytes)` -- a rank-1 array of byte values (`0..256`) -- or `err(...)`. Binary formats round-trip through `decode_bytes`/`tokenize_bytes` (bytes to/from text) and `parse_json`/`to_json`: `parse_json(decode_bytes(unwrap(read_bytes(p))))`. |
