...
pub fn eval_clone_model(args: &[Expr], env: &mut Environment) -> Result<ModelSpec, EvalError> {
    mlpl_models_mutate::clone_model_inner(args, env, clone_resolver)
}

/// `perturb_params(m, family, sigma, seed)` entry point.
pub fn eval_perturb_params(args: &[Expr], env: &mut Environment) -> Result<DenseArray, EvalError> {
