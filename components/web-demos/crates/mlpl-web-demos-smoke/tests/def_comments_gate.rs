...
fn every_demo_def_has_a_doc_string_and_parses() {
    ...
    for demo in DEMOS.iter() {
        for (i, line) in demo.lines.iter().enumerate() {
            ...
            checked += 1;
            let brace = t.find('{').unwrap_or_else(|| {
                panic!("[{} line {i}] def without a body: {t}", demo.name)
            });
            ...
            let toks = lex(line)
                .unwrap_or_else(|e| panic!("[{} line {i}] lex: {e:?}", demo.name));
            parse(&toks)
                .unwrap_or_else(|e| panic!("[{} line {i}] parse: {e:?}", demo.name));
        }
    }
