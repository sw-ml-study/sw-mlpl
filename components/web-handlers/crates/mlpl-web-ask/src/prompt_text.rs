...
define a function with `def u:name(a, b) { body }` -- the `u:` prefix is REQUIRED; the block's last expression is its value (`return expr` also works for early exit) -- then call it `u:name(args)`; iterate with `for x in range(n) { ... }` or `while cond { ... }`; index/slice a tensor with `take(x, axis, i)`; trap a hard error with `try { ... } catch e { ... }` (e is {kind, message}); propagate a Result with postfix `?` inside a `def u:` body. \
