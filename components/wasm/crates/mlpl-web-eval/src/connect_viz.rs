...
/// array `shape` (+ optional flat `values` / `string_list`), so a
/// server-evaluated result renders as a 3D sculpture. `program`
/// supplies the label and the `name =` target. No-op when there is
/// no shape (a string / `:ask` reply).
pub(crate) fn emit_from_response(program: &str, body: &Json) {
    ...
    let name = program
        ...
        .to_string();
    let info =
        mlpl_web_viz3d::events::build_shape_info_full(name, shape, values, string_list, None);
