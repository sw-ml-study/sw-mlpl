...
pub struct OnboardingState {
    ...
    /// Saga 82: the host crate's `env!("CARGO_PKG_VERSION")`
    /// (mlpl-web carries that env var; the new render-shell
    /// crate doesn't) threaded down so the "what's new" close
    /// callback can persist it as the last-seen version.
