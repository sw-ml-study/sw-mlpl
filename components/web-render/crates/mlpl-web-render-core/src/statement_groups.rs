...
/// (docs/apl2-idioms.mlpl) keeps it in the transcript, and a
/// trailing comment-only group (a closing summary) is emitted
/// for narration. A trailing unbalanced group is still emitted
/// (the evaluator reports the unclosed delimiter).
#[must_use]
