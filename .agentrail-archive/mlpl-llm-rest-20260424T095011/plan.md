# Saga 19: LLM-as-Tool REST Integration (v0.15.0)
...
1. Bump `Cargo.toml` workspace.package.version
   `0.14.1 -> 0.15.0`. Minor-level bump because
   new language surface (not a patch).
2. `CHANGELOG.md`: v0.15.0 section above v0.14.1.
   Document `llm_call`, the demo, the `:ask`
   migration, the CLI-only scope, and the
   explicit deferred list.
3. `docs/saga.md`: Saga 19 retrospective block
   above Saga 18 (or below Saga 21 placeholder;
   pick the ordering that matches existing
...
9. Tag `v0.15.0` locally; do NOT push without
   explicit user confirmation (v0.12.0 / v0.13.0 /
   v0.14.0 / v0.14.1 cadence).
